...
class SendBroadcastNotification implements ShouldQueue
{
        ...
        try {
            ...
            foreach ($this->notification->target_types as $targetType) {
                $sent = match ($targetType) {
                    ...
                    Notification::TARGET_CENTERS => $this->sendTocenters($fcmService),
                    default => 0,
                };
     ...
     */
    private function sendTocenters(FcmService $fcmService): int
    {
        $tokens = Device::where('owner_type', Center::class)
            ->whereNotNull('fcm_token')
            ->pluck('fcm_token')
            ->filter()
            ->unique()
